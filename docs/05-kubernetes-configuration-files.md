...
```bash
{
  kubectl config set-cluster kubernetes-the-hard-way \
    --certificate-authority=pki/ca/ca.crt \
    --embed-certs=true \
    --server=https://kube-cp-01.kubernetes.local:6443 \
    --kubeconfig=kube-proxy.kubeconfig

  kubectl config set-credentials system:kube-proxy \
    --client-certificate=pki/cp/kube-proxy.crt \
    --client-key=pki/cp/kube-proxy.key \
    --embed-certs=true \
    --kubeconfig=kube-proxy.kubeconfig
...
```bash
{
  ...
  kubectl config set-credentials system:kube-controller-manager \
    --client-certificate=pki/cp/kube-controller-manager.crt \
    --client-key=pki/cp/kube-controller-manager.key \
    --embed-certs=true \
    --kubeconfig=kube-controller-manager.kubeconfig
...
```bash
{
  ...
  kubectl config set-credentials system:kube-scheduler \
    --client-certificate=pki/cp/kube-scheduler.crt \
    --client-key=pki/cp/kube-scheduler.key \
    --embed-certs=true \
    --kubeconfig=kube-scheduler.kubeconfig
...
```bash
{
  ...
  kubectl config set-credentials admin \
    --client-certificate=pki/cp/admin.crt \
    --client-key=pki/cp/admin.key \
    --embed-certs=true \
    --kubeconfig=admin.kubeconfig
